...
test("website copy presents ask and explain as paired directions", () => {
  ...
  assert.match(source, /const activeDirectionVerb = isDaiyu/);
  assert.match(source, /\? "释颦"/);
  assert.match(source, /: "拟颦"/);
  assert.match(source, /\? "释礼"/);
  assert.match(source, /: "问礼"/);
  assert.match(source, /今日还可\$\{activeDirectionVerb\}/);
  assert.match(source, /分钟后再\$\{activeDirectionVerb\}/);
});
...
test("card export uses direction-aware title, footer, and filename semantics", () => {
  ...
  assert.match(source, /\? `释颦-\$\{levelTitle\}`/);
  assert.match(source, /: `拟颦-\$\{levelTitle\}`/);
  assert.match(source, /\? `释礼-\$\{levelTitle\}`/);
  assert.match(source, /: `问礼-\$\{levelTitle\}`/);
});
